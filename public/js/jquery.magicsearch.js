    ...
    MagicSearch.prototype = {
        ...
        searchData: function searchData(isAll, isScroll) {
            ...
            if (data.length === 0) {
                ...
                htmlStr += '<ul style="overflow-y:scroll;height:200px;">';
                data.forEach(function (item, index) {
        ...
        appendHtml: function appendHtml(data) {
            ...
            var $item = $('<div class="' + doms.item + '" data-id="' + data[options.id] + '" title="' + formatParse(options.format, data) + '"><span id="MY_' + data[options.id] + '" data-symbol="' + formatParse(options.format, data) + '"  title="' + data['stockName'] + '">' + formatParse(SEPARATOR + options.multiField + SEPARATOR, data) + '</span><a class="' + doms.close + '" data-id="' + data[options.id] + '" href="javascript:;"></a></div>');
            $item.css({
